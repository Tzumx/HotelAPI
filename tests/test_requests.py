...
def test_routers_requests(client):
    """Test routers endpoints for requests"""
    ...
    with patch('crud.requests.filter_requests') as mock:
        mock.return_value = [requests_schema.RequestInfo(
            **(request_data | {"id": 11})), ]
        response = client.post('/requests/filter', json={})
        assert response.status_code == 200
        data = response.json()
    ...
    with patch('crud.requests.update_request') as mock:
        ...
        response = client.put('/requests/11', json={"price": 33})
        assert response.status_code == 200
        data = response.json()
    ...
    with patch('crud.requests.delete_request') as mock:
        ...
        response = client.delete('/requests/11')
        assert response.status_code == 200
        data = response.json()
...
def test_crud_requests(client):
    ...
    response = client.put(f'/requests/{request_id}', json={'price': 33})
    assert response.status_code == 200
    response = client.post('/requests/filter', json={'price': 33})
    data = response.json()
    right_item = False
    for item in data:
    ...
    assert response.status_code == 200
    response = client.post('/requests/filter')
    data = response.json()
    right_item = True
    for item in data:
        if item['id'] == request_id:
    ...
    response = client.delete(f'/requests/{request_id+888}')
