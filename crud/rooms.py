...
async def add_feature_to_roomtype(type_id: int, feature_id: int):
    ...
    query = rooms_model.roomtype_feature.select(
                ).where(rooms_model.roomtype_feature.c.type_id == type_id
                ).where(rooms_model.roomtype_feature.c.feature_id == feature_id)
    answer = await database.execute(query)
    if answer != None:
    ...
    sum_table = rooms_model.roomtype_feature.join(rooms_model.room_type,
                rooms_model.roomtype_feature.c.type_id == rooms_model.room_type.c.id
                ).join(rooms_model.feature, 
                rooms_model.roomtype_feature.c.feature_id == rooms_model.feature.c.id)
    result = await database.fetch_one(sum_table.select().
                                      where(rooms_model.room_type.c.id == type_id).
                                      where(rooms_model.feature.c.id == feature_id).
                                      with_only_columns(
...
async def delete_feature_from_roomtype(type_id: int, feature_id: int):
    ...
    query = rooms_model.roomtype_feature.select().where(
        rooms_model.oomtype_feature.c.type_id == type_id
    ).where(
        rooms_model.roomtype_feature.c.feature_id == feature_id)
    answer = await database.execute(query)
    if answer != None:
        query = rooms_model.roomtype_feature.delete().where(
            rooms_model.roomtype_feature.c.type_id == type_id
        ).where(
            rooms_model.roomtype_feature.c.feature_id == feature_id)
        await database.execute(query)
        answer = "Success"
...
async def create_room(room: rooms_schema.RoomCreate):
    ...
    answer = await database.execute(query)
    if answer == None:
        query = rooms_model.room.insert().values(
